...
private:
    static bool canFinish(vector<int> &piles, int k, int h)
    {
        long long hours = 0;
        for (int i = 0; i < piles.size(); i++)
        {
            int pile = piles[i];
            hours += (pile + (long long)k - 1) / k; // ceil(pile / k)
            if (hours > h)
                return false;
        }
        return true;
...
{
    int arr[] = {3, 6, 7, 11};
    vector<int> piles(arr, arr + sizeof(arr) / sizeof(arr[0]));
    int h = 8;
    cout << KokoEatingBananas::minEatingSpeed(piles, h) << endl; // Output: 4
    return 0;
}
