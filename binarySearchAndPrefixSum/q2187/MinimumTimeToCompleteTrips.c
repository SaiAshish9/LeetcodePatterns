...
bool canCompleteTrips(int time[], int n, long long currentTime, int totalTrips) {
    long long tripsDone = 0;
    for (int i = 0; i < n; i++) {
        tripsDone += currentTime / time[i];
...
long long minimumTime(int time[], int n, int totalTrips) {
    int minTime = INT_MAX;
    for (int i = 0; i < n; i++) {
    ...
    while (left < right) {
        ...
        if (canCompleteTrips(time, n, mid, totalTrips)) {
...
int main() {
    ...
    printf("%lld\n", minimumTime(time, n, totalTrips)); // Output: 3
    return 0;
}
