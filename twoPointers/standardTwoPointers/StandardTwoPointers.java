...
package twoPointers.standardTwoPointers;

public class StandardTwoPointers {

    private static String findPair(int[] nums, int target) {
        int left = 0;
        ...
        while (left < right) {
            ...
            if (sum == target) {
                return "Pair found: (" + nums[left] + ", " + nums[right] + ")";
            } else if (sum < target) {
                left++;
            } else {
                right--;
    ...
    public static void main(String[] args) {
        int nums[] = { 2, 7, 11, 15 };
        int target = 9;
        System.out.println(findPair(nums, target)); // Output: Pair found: (2, 7)
    }
